...
<button 
      ...
      @click="downloadFile"
      :class="{ 'btn-disabled': !res.ppt_download && !res.lesson_download }"
      >
      <span class="iconfont icon-xiazai"></span>
      <span>下载{{ activeTab === 'ppt' ? '课件' : '教案' }}</span>
    </button>
  </div>
  <button 
    class="preview-tab-btn"
...
// 初始化 res 时，优先从 query 参数里取值
const res = ref({
  ppt_download: '',
  lesson_download: ''
});

// 🌟 方案：使用 watch 深度监听路由参数（最稳妥）
import { watch } from 'vue';

watch(() => route.query, (newQuery) => {
  if (newQuery.session_id) sessionId.value = newQuery.session_id as string;
  console.log("监听到 Query 参数变化:", newQuery);
  if (newQuery.ppt_url) {
    res.value.ppt_download = newQuery.ppt_url as string;
  }
  if (newQuery.lesson_url) {
    res.value.lesson_download = newQuery.lesson_url as string;
  }
...
const refreshIframePreview = (url: string) => {
  if (pptFrameRef.value) {
    // 确保 url 包含 http，如果没有则拼接 baseUrl
    const fullUrl = url.startsWith('http') ? url : `${baseUrl}${url}`;
    // 加上时间戳 t=... 强制浏览器跳过缓存
    const finalUrl = `${fullUrl}${fullUrl.includes('?') ? '&' : '?'}t=${Date.now()}`;
    
    pptFrameRef.value.innerHTML = `
      <iframe src="${finalUrl}" width="100%" height="100%" style="border:none; border-radius:16px;"></iframe>
    `;
  }
}
const sendMessage = async () => {
  ...
  try {
    // 🌟 修改点：发送 JSON 数据，包含 session_id 和 active_tab
    const response = await fetch(`${baseUrl}/api/v1/update-content`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: userMsg,
        session_id: sessionId.value, // 必传：告诉后端改哪个文件
        active_tab: activeTab.value,   // 必传：告诉后端是改 PPT 还是教案
        // 如果有附件需求，建议另开文件上传接口，或者保持 FormData 形式
      })
    });
    ...
    // 展现 AI 的回复
    chatMessages.value.push({ role: 'assistant', content: data.reply || '已更新' });

    // 🌟 核心刷新逻辑
    if (activeTab.value === 'ppt' && data.ppt_download) {
      res.value.ppt_download = data.ppt_download;
      // 强制刷新 Iframe，带上时间戳规避浏览器缓存
      refreshIframePreview(data.ppt_download); 
    } 
    
    if (data.lesson_content) {
      // 🌟 如果后端返回了新的教案文本，直接刷新 A4 纸预览
      setLessonData(data.lesson_content); 
      if (data.lesson_download) res.value.lesson_download = data.lesson_download;
    }

  } catch (error) {
    chatMessages.value.push({ role: 'assistant', content: '更新失败，请重试' });
  } finally {
    isAILoading.value = false;
    await scrollChatToBottom();
  }
}
// 1. 定义存储后端返回数据的对象 (确保这里能拿到生成接口返回的 JSON)

// 2. 基础配置：指向你的 FastAPI 地址
const baseUrl = "http://localhost:8000"; 

// 3. 定义通用的触发下载函数
const triggerDownload = async (path: string, defaultName: string) => {
  console.log("触发下载，传入的 path 为:", path); 
  if (!path) {
    alert("下载失败：路径为空，参数未传达");
    return;
  }
  const fullUrl = `${baseUrl}${path}`;
  console.log("最终拼接的完整 URL:", fullUrl);
  try {
    // 拼接完整请求地址
    ...
    link.href = url;
    link.download = defaultName; // 用户保存到本地时的文件名
    ...
    alert("下载失败，请检查后端服务是否正常运行");
  }
};

// 4. 实现你在模板中绑定的 @click="downloadFile"
const downloadFile = () => {
  // 根据当前选中的选项卡（activeTab）决定下载哪一个
  if (activeTab.value === 'ppt') {
    triggerDownload(res.value.ppt_download, '教学课件.pptx');
  } else if (activeTab.value === 'lesson') {
    triggerDownload(res.value.lesson_download, '教学教案.docx');
  } else {
    // 如果在游戏页点击，默认询问
    if(confirm("确定下载 PPT 课件吗？")) {
      triggerDownload(res.value.ppt_download, '教学课件.pptx');
    }
  }
};
...
onMounted(async () => {
  console.log("收到路由参数:", route.query); // 检查控制台有没有这两个值

  if (route.query.ppt_url) {
    res.value.ppt_download = route.query.ppt_url as string;
  }
  if (route.query.lesson_url) {
    res.value.lesson_download = route.query.lesson_url as string;
  }
  console.log("当前下载路径库:", res.value);
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      const response = await fetch(res.value.lesson_download)
      if (response.ok) {
        const blob = await response.blob()
        const defaultDocx = new File([blob],res.value.lesson_download, { type: blob.type })
