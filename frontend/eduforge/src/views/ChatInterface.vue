<!-- ChatInterface.vue - 完整修改版，修复图标和记录点击问题 -->
<template>
        ...
        <div class="messages-container" ref="messagesContainer">
          <div v-if="messages.length === 0" class="welcome-message">
            ...
            <div class="feature-cards">
              <div class="card" @click="setPrompt('请帮我生成一份关于分数的PPT课件')">
              ...
              <div class="card" @click="setPrompt('请帮我写一份小学语文《静夜思》的教案')">
                <div class="card-img-wrapper">
              ...
              <div class="card" @click="setPrompt('请制作一个展示光合作用过程的教学GIF')">
                <div class="card-img-wrapper">
  ...
  onMounted(() => {
    ...
    link.rel = 'stylesheet'
    link.href = '//at.alicdn.com/t/c/font_5134397_sbsapm81i9.css'
  ...
  // 记录最后一次生成的内容类型
  const lastGeneratedType = ref('ppt')
  
  // 跳转到对应的编辑器
  const goToPreview = () => {
    console.log('正在跳转到编辑器...', {
      type: lastGeneratedType.value,
    ...
    try {
      ...
      if (!routes.includes(path) && !routes.includes(path + '/')) {
        console.warn('路径不存在，使用默认路径')
        path = '/editor'
      }
      
      const query = {
        title: userInput.value || '新内容',
        type: lastGeneratedType.value,
        t: Date.now().toString()
      }
      ...
      router.push({
        ...
        console.log('跳转成功到:', path)
      }).catch((err) => {
        console.error('router.push 失败:', err)
        const url = `${path}?title=${encodeURIComponent(userInput.value || '新内容')}&type=${lastGeneratedType.value}&t=${Date.now()}`
        console.log('使用备用跳转:', url)
        window.location.href = url
      })
    } catch (error) {
      console.error('跳转出错:', error)
      if (lastGeneratedType.value === 'gif') {
        window.location.href = '/gif-editor?title=' + encodeURIComponent(userInput.value || '新动画') + '&type=gif&t=' + Date.now()
      } else if (lastGeneratedType.value === 'lesson') {
        window.location.href = '/lesson-editor?title=' + encodeURIComponent(userInput.value || '新教案') + '&type=lesson&t=' + Date.now()
      } else {
        window.location.href = '/editor?title=' + encodeURIComponent(userInput.value || '新课件') + '&type=ppt&t=' + Date.now()
      }
    }
  }
  ...
  * {
  ...
  /* 当侧边栏展开时，logo-area 居中 */
  .sidebar:not(.collapsed) .logo-area {
    flex: 1;
    justify-content: center;
  }
  
  /* 当侧边栏收起时，logo-area 左对齐 */
