...
//   });
// }
const downloadPaths = ref({ ppt: '', lesson: '' });
...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch('api/v1/generate-content', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        subject: userMessage.content,
        stage: "generate",
      })
    });
    
    const data = await response.json();
    currentSessionId.value = data.session_id || data.conversation_id || '';
    // 存下路径（如果后端没给，留空）
    downloadPaths.value.ppt = data.ppt_download || '';
    downloadPaths.value.lesson = data.lesson_download || '';
  } catch (err) {
  ...
  setTimeout(async () => {
    ...
    if (scriptItem) {
      messages.value.push({ role: 'assistant', content: scriptItem.reply });
      currentStep.value++;
      await scrollToBottom();

      // 3. 触发跳转
      if (scriptItem.shouldJump) {
        // 延迟 2 秒跳转，给用户看一眼“正在生成”的消息
        setTimeout(() => {
          // 🌟 关键：直接调用那个带参数的函数
          jumpToPreviewWithData();
        }, 2000); 
      }
    }
  }, 1500);

  // ❌ 注意：这里后面不要再写任何 setTimeout 或者 generateAllThree 的调用了！
};
// const jumpToPreviewWithData = () => {
//   // 1. 显式定义字符串，避免任何变量污染
//   const finalPptPath = '/api/download/ppt_1775046869.pptx';
//   const finalLessonPath = '/api/download/lesson_1775046869.docx';

//   console.log("准备跳转，目标路径:", '/preview-layout');
//   console.log("携带参数:", { ppt: finalPptPath, lesson: finalLessonPath });

//   // 2. 使用这种更直观的写法，确保 query 对象被完整识别
//   const jumpQuery = {
//     title: '操作系统进程调度-测试版',
//     type: 'all',
//     ppt_url: finalPptPath,    // 👈 重点检查这两个 Key
//     lesson_url: finalLessonPath,
//     t: Date.now().toString()
//   };
...
//     query: jumpQuery
//   }).then(() => {
//     console.log("路由跳转执行成功，最终 URL 包含参数:", jumpQuery);
//   }).catch(err => {
...
const jumpToPreviewWithData = () => {
  // 1. 动态获取后端返回的真实路径
  // 如果 downloadPaths.value.ppt 有值，就用后端的；否则用硬编码演示
  const finalPptPath = downloadPaths.value.ppt || '/api/download/ppt_1775046869.pptx';
  const finalLessonPath = downloadPaths.value.lesson || '/api/download/lesson_1775046869.docx';

  console.log("🚀 准备跳转预览页...");
  console.log("🔗 PPT真实路径:", finalPptPath);
  console.log("🔗 教案真实路径:", finalLessonPath);

  // 2. 执行跳转并携带 Query 参数
  router.push({
    path: '/preview-layout',
    query: {
      title: '高效PPT制作技巧', // 这里可以根据 data.title 动态获取
      type: 'all',
      ppt_url: finalPptPath, 
      lesson_url: finalLessonPath,
      session_id: currentSessionId.value,
      t: Date.now().toString() // 防止浏览器缓存
    }
  });
};
